...
/*   Updated: 2023/01/11 14:56:04 by kdhrif           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_mode_infile(t_pipex *pipex, int *i)
{
	...
	{
		if (pipex->argc < 6)
			generic_err(pipex, "Wrong numbers of arguments.(check_mode_infile)"\
					, 0, EXIT_FAILURE);
		here_doc(pipex);
		pipex->heredoc = 1;
		*i = 3;
	...
	else
	{
		...
		*i = 2;
		if (dup2(pipex->fd_file1, 0) == -1)
			generic_err(pipex, "Dup2 error. (check_mode_infile)", 1, EXIT_FAILURE);
	}
}
...
{
	if (pipe(pipex->fd) == -1)
		generic_err(pipex, "Pipe error. (here_doc)", 1, EXIT_FAILURE);
	pipex->pid = fork();
	if (pipex->pid == -1)
		generic_err(pipex, "Fork error. (here_doc)", 1, EXIT_FAILURE);
	if (pipex->pid == 0)
		put_heredoc(pipex);
	else
	{
		...
		if (dup2(pipex->fd[0], STDIN_FILENO) == -1)
			generic_err(pipex, "Dup2 error. (here_doc)", 1, EXIT_FAILURE);
...
void	put_heredoc(t_pipex *pipex)
{
	...
	while (1)
	{
		...
		if (line == NULL)
			generic_err(pipex, "Get next line failed. (put_heredoc)"\
					, 0, EXIT_FAILURE);
